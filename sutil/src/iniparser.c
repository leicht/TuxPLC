/*
 ...
 Added some functions, by Stephane Jeanne
...
char *iniparser_getsectionname(dictionary *d,int n)
{	char *res;
	if (n>=d->n) return(NULL);
	char *str=d->key[n];
	res=strchr(str,':');
	if (res==NULL) res=strdup(str); else
	{
		//printf("%p : %p",str,res);
		res=strndup(str,res-str);
	}
	return(res);
}
char *iniparser_getkeyname(dictionary *d,int n)
{	char *res;
	if (n>=d->n) return(NULL);
	char *str=d->key[n];
	res=strchr(str,':');
...
{	int i,result=0;
	char *s= iniparser_getsecname(d,n);
	if (s==NULL) return(0);
	char *section;
	if (asprintf (&section, "%s:", s) < 0) return(-1);
	for(i=0;i<d->n;i++)
	{
		if (strncmp(section,d->key[i],strlen(section))==0) result++;
	}
	free(section);
	return(result);
}
