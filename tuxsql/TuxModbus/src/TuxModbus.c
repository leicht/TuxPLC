 ...
 *           Stephane LEICHT    stephane.leicht@                           *
 *                                                                         *
...
char *TuxGetErrMsg(int s_err_type,int s_errno)
{
 switch (s_err_type){
  ...
  default :return TuxGetInternalErrMsg(s_errno);//Df1_GetDf1ErrMsg(s_errno);
 }
}
...
int _GetPlc(MYSQL *db,LISTE *plcs,char *plcname)
{
	...
	and d.TAG_SYSTEM='MDB' and p.PLCNAME='%s'",plcname);
	else res=asprintf(&sel_str,"SELECT distinct p.* FROM MODBUS as p LEFT JOIN DEFINITION as d ON p.PLCNAME=d.PLCNAME where p.PLC_ENABLE=1 and p.PLC_TTY is not null and d.ADDRESS is not null and d.READING=1 and d.TAG_SYSTEM='MDB'");
	if (res<0)
	{
	...
	while ((row = mysql_fetch_row(SqlResult)))
	{
		...
		if (plc!=NULL)
		{
			...
			//snprintf(plc->PlcPath,lengths[1]+1,"%s",row[1]);
                        snprintf(plc->structModbus.ip,lengths[1]+1,"%s",row[1]);
                        snprintf(plc->structModbus.device,lengths[2]+1,"%s",row[2]);
			Log(LOG_NOTICE,"speed : %d\n",atoi(row[3]));
			//plc->PlcSpeed=atoi(row[2]);
                        plc->structModbus.baud=atoi(row[3]);
                        snprintf(plc->structModbus.parity,lengths[4]+1,"%s",row[4]);
                        plc->structModbus.data_bit=atoi(row[5]);
                        plc->structModbus.stop_bit=atoi(row[6]);
                        plc->DeviceId=atoi(row[7]);
...
int BuildSocket(PLC *plc)
{
	if (plc->socket>=0) return(plc->socket);
        Log(LOG_DEBUG,"modbus_init (%s) created for PLC : %s(%s)\n",plc->structModbus.device,plc->PlcName,plc->PlcPath);
        modbus_init_rtu(&plc->structModbus, plc->structModbus.device, plc->structModbus.baud, plc->structModbus.parity, plc->structModbus.data_bit, plc->structModbus.stop_bit);
        Log(LOG_DEBUG,"modbus_init (%s) created for PLC : %s(%s)\n",plc->structModbus.device,plc->PlcName,plc->PlcPath);
        if (modbus_connect(&plc->structModbus) == -1) {
	...
	//plc->socket=Df1_open_device(plc->PlcPath, plc->PlcSpeed,0,8,1); //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        //if (plc->socket>=0)
	{
            plc->socket=plc->structModbus.fd;
            Log(LOG_DEBUG,"Socket (%d) created for PLC : %s(%s)\n",plc->socket,plc->PlcName,plc->PlcPath);
            return(plc->socket);
	}
}
...
void KillAll(LISTE *plcs)
{
	...
	while (elt!=NULL)
	{
		...
		{
			plc->socket=-1;
			modbus_close(&plc->structModbus);//Df1_close_device(plc->socket);
		}
		elt=GetNext(plcs,elt);
	}
}
...
int ReadTag(PLC *plc,TAG *tag)
{
	...
	int error;
        ...
        if ((error = read_holding_registers(&plc->structModbus, plc->DeviceId,atoi(tag->Address),2, tab_rp_registers))<0) {
